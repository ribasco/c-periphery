...
void test_arguments(void) {
    ...
    /* Invalid direction */
    passert(cp_gpio_open(gpio, device, pin_input, 5) == GPIO_ERROR_ARG);
...
void test_open_config_close(void) {
    ...
    passert(cp_gpio_open(gpio, device, -1, GPIO_DIR_IN) == GPIO_ERROR_OPEN);
    passert(cp_gpio_errno(gpio) == EINVAL);

    /* Open legitimate GPIO */
    passert(cp_gpio_open(gpio, device, pin_output, GPIO_DIR_IN) == 0);

    /* Check properties */
    passert(cp_gpio_line(gpio) == pin_output);
    passert(cp_gpio_fd(gpio) >= 0);
    passert(cp_gpio_chip_fd(gpio) >= 0);

    /* Invalid direction */
    passert(cp_gpio_set_direction(gpio, 5) == GPIO_ERROR_ARG);
    /* Invalid interrupt edge */
    passert(cp_gpio_set_edge(gpio, 5) == GPIO_ERROR_ARG);

    /* Set direction out, check direction out, check value low */
    passert(cp_gpio_set_direction(gpio, GPIO_DIR_OUT) == 0);
    passert(cp_gpio_get_direction(gpio, &direction) == 0);
    passert(direction == GPIO_DIR_OUT);
    passert(cp_gpio_read(gpio, &value) == 0);
    passert(value == false);
    /* Set direction out low, check direction out, check value low */
    passert(cp_gpio_set_direction(gpio, GPIO_DIR_OUT_LOW) == 0);
    passert(cp_gpio_get_direction(gpio, &direction) == 0);
    passert(direction == GPIO_DIR_OUT);
    passert(cp_gpio_read(gpio, &value) == 0);
    passert(value == false);
    /* Set direction out high, check direction out, check value high */
    passert(cp_gpio_set_direction(gpio, GPIO_DIR_OUT_HIGH) == 0);
    passert(cp_gpio_get_direction(gpio, &direction) == 0);
    passert(direction == GPIO_DIR_OUT);
    passert(cp_gpio_read(gpio, &value) == 0);
    passert(value == true);
    /* Attempt to set interrupt edge on output GPIO */
    passert(cp_gpio_set_edge(gpio, GPIO_EDGE_RISING) == GPIO_ERROR_INVALID_OPERATION);
    /* Attempt to read event on output GPIO */
    passert(cp_gpio_read_event(gpio, &edge, NULL) == GPIO_ERROR_INVALID_OPERATION);

    /* Set direction in, check direction in */
    passert(cp_gpio_set_direction(gpio, GPIO_DIR_IN) == 0);
    passert(cp_gpio_get_direction(gpio, &direction) == 0);
    passert(direction == GPIO_DIR_IN);
    passert(cp_gpio_read(gpio, &value) == 0);

    /* Set edge none, check edge none */
    passert(cp_gpio_set_edge(gpio, GPIO_EDGE_NONE) == 0);
    passert(cp_gpio_get_edge(gpio, &edge) == 0);
    passert(edge == GPIO_EDGE_NONE);
    /* Set edge rising, check edge rising */
    passert(cp_gpio_set_edge(gpio, GPIO_EDGE_RISING) == 0);
    passert(cp_gpio_get_edge(gpio, &edge) == 0);
    passert(edge == GPIO_EDGE_RISING);
    /* Set edge falling, check edge falling */
    passert(cp_gpio_set_edge(gpio, GPIO_EDGE_FALLING) == 0);
    passert(cp_gpio_get_edge(gpio, &edge) == 0);
    passert(edge == GPIO_EDGE_FALLING);
    /* Set edge both, check edge both */
    passert(cp_gpio_set_edge(gpio, GPIO_EDGE_BOTH) == 0);
    passert(cp_gpio_get_edge(gpio, &edge) == 0);
...
void *gpio_poll_thread(void *arg) {
    ...
    intptr_t ret = cp_gpio_poll(gpio, timeout_ms);
...
void test_loopback(void) {
    ...
    passert(cp_gpio_open(gpio_in, device, pin_input, GPIO_DIR_IN) == 0);
    passert(cp_gpio_open(gpio_out, device, pin_output, GPIO_DIR_OUT) == 0);

    /* Drive out low, check in low */
    passert(cp_gpio_write(gpio_out, false) == 0);
    passert(cp_gpio_read(gpio_in, &value) == 0);
    passert(value == false);

    /* Drive out high, check in high */
    passert(cp_gpio_write(gpio_out, true) == 0);
    passert(cp_gpio_read(gpio_in, &value) == 0);
    passert(value == true);

    /* Check poll falling 1 -> 0 interrupt */
    passert(cp_gpio_set_edge(gpio_in, GPIO_EDGE_FALLING) == 0);
    gpio_poll_start(&poll_thread, gpio_in, 1000);
    passert(cp_gpio_write(gpio_out, false) == 0);
    passert(gpio_poll_join(poll_thread) == 1);
    passert(cp_gpio_read(gpio_in, &value) == 0);
    passert(value == false);
    passert(cp_gpio_read_event(gpio_in, &edge, NULL) == 0);
    passert(edge == GPIO_EDGE_FALLING);

    /* Check poll rising 0 -> 1 interrupt */
    passert(cp_gpio_set_edge(gpio_in, GPIO_EDGE_RISING) == 0);
    gpio_poll_start(&poll_thread, gpio_in, 1000);
    passert(cp_gpio_write(gpio_out, true) == 0);
    passert(gpio_poll_join(poll_thread) == 1);
    passert(cp_gpio_read(gpio_in, &value) == 0);
    ...
    /* Set both edge */
    passert(cp_gpio_set_edge(gpio_in, GPIO_EDGE_BOTH) == 0);
    ...
    /* Check poll timeout */
    passert(cp_gpio_poll(gpio_in, 1000) == 0);

    passert(cp_gpio_close(gpio_in) == 0);
    passert(cp_gpio_close(gpio_out) == 0);

    /* Free GPIO */
    cp_gpio_free(gpio_in);
...
void test_interactive(void) {
    ...
    passert(cp_gpio_open(gpio, device, pin_output, GPIO_DIR_OUT) == 0);
    ...
    /* Check tostring */
    passert(cp_gpio_tostring(gpio, str, sizeof(str)) > 0);
    printf("GPIO description: %s\n", str);
    printf("GPIO description looks OK? y/n\n");
    passert(getc_yes());

    /* Drive GPIO out low */
    passert(cp_gpio_write(gpio, false) == 0);
    printf("GPIO out is low? y/n\n");
    passert(getc_yes());

    /* Drive GPIO out high */
    passert(cp_gpio_write(gpio, true) == 0);
