...
void test_arguments(void) {
    ...
    /* Invalid direction */
    passert(cp_gpio_open_sysfs(gpio, pin_input, 5) == GPIO_ERROR_ARG);
...
void test_open_config_close(void) {
    ...
    passert(cp_gpio_open_sysfs(gpio, -1, GPIO_DIR_IN) == GPIO_ERROR_OPEN);
    passert(cp_gpio_errno(gpio) == EINVAL);

    /* Open legitimate GPIO */
    passert(cp_gpio_open_sysfs(gpio, pin_output, GPIO_DIR_IN) == 0);
    ...
    passert(cp_gpio_chip_fd(gpio) == GPIO_ERROR_UNSUPPORTED);
    /* Unsupported method */
    passert(cp_gpio_read_event(gpio, &edge, NULL) == GPIO_ERROR_UNSUPPORTED);
...
void test_loopback(void) {
    ...
    passert(cp_gpio_open_sysfs(gpio_in, pin_input, GPIO_DIR_IN) == 0);
    passert(cp_gpio_open_sysfs(gpio_out, pin_output, GPIO_DIR_OUT) == 0);
...
void test_interactive(void) {
    ...
    passert(cp_gpio_open_sysfs(gpio, pin_output, GPIO_DIR_OUT) == 0);
