...
void test_arguments(void) {
    ...
    /* Invalid mode */
    passert(cp_spi_open(spi, device, 4, 1e6) == SPI_ERROR_ARG);
    /* Invalid bit order */
    passert(cp_spi_open_advanced(spi, device, 0, 1e6, LSB_FIRST + 1, 8, 0) == SPI_ERROR_ARG);
...
void test_open_config_close(void) {
    ...
    passert(cp_spi_open(spi, device, 0, 100000) == 0);

    /* Confirm bit_order = MSB first, bits_per_word = 8, extra_flags = 0 */
    passert(cp_spi_get_bit_order(spi, &bit_order) == 0);
    passert(bit_order == MSB_FIRST);
    passert(cp_spi_get_bits_per_word(spi, &bits_per_word) == 0);
    passert(bits_per_word == 8);
    passert(cp_spi_get_extra_flags(spi, &extra_flags) == 0);
    ...
    /* Try modes 1,2,3,0 */
    passert(cp_spi_set_mode(spi, 1) == 0);
    passert(cp_spi_get_mode(spi, &mode) == 0);
    passert(mode == 1);
    passert(cp_spi_set_mode(spi, 2) == 0);
    passert(cp_spi_get_mode(spi, &mode) == 0);
    passert(mode == 2);
    passert(cp_spi_set_mode(spi, 3) == 0);
    passert(cp_spi_get_mode(spi, &mode) == 0);
    passert(mode == 3);
    passert(cp_spi_set_mode(spi, 0) == 0);
    passert(cp_spi_get_mode(spi, &mode) == 0);
    passert(mode == 0);

    /* Try 100KHz, 500KHz, 1MHz */
    passert(cp_spi_set_max_speed(spi, 100000) == 0);
    passert(cp_spi_get_max_speed(spi, &max_speed) == 0);
    passert(max_speed == 100000);
    passert(cp_spi_set_max_speed(spi, 500000) == 0);
    passert(cp_spi_get_max_speed(spi, &max_speed) == 0);
    passert(max_speed == 500000);
    passert(cp_spi_set_max_speed(spi, 1000000) == 0);
    passert(cp_spi_get_max_speed(spi, &max_speed) == 0);
    passert(max_speed == 1000000);

    passert(cp_spi_close(spi) == 0);
...
void test_loopback(void) {
    ...
    passert(cp_spi_transfer(spi, buf, buf, sizeof(buf)) == 0);
...
void test_interactive(void) {
    ...
    /* Check tostring */
    passert(cp_spi_tostring(spi, str, sizeof(str)) > 0);
    ...
    getc(stdin);
    passert(cp_spi_transfer(spi, buf, NULL, sizeof(buf)) == 0);
