...
/* Primary Functions */
cp_spi_t *spi_new(void);
int spi_open(cp_spi_t *spi, const char *path, unsigned int mode, uint32_t max_speed);
int spi_open_advanced(cp_spi_t *spi, const char *path, unsigned int mode,
                        uint32_t max_speed, spi_bit_order_t bit_order,
                        uint8_t bits_per_word, uint8_t extra_flags);
int spi_transfer(cp_spi_t *spi, const uint8_t *txbuf, uint8_t *rxbuf, size_t len);
int spi_close(cp_spi_t *spi);
void spi_free(cp_spi_t *spi);

/* Getters */
int spi_get_mode(cp_spi_t *spi, unsigned int *mode);
int spi_get_max_speed(cp_spi_t *spi, uint32_t *max_speed);
int spi_get_bit_order(cp_spi_t *spi, spi_bit_order_t *bit_order);
int spi_get_bits_per_word(cp_spi_t *spi, uint8_t *bits_per_word);
int spi_get_extra_flags(cp_spi_t *spi, uint8_t *extra_flags);

/* Setters */
int spi_set_mode(cp_spi_t *spi, unsigned int mode);
int spi_set_max_speed(cp_spi_t *spi, uint32_t max_speed);
int spi_set_bit_order(cp_spi_t *spi, spi_bit_order_t bit_order);
int spi_set_bits_per_word(cp_spi_t *spi, uint8_t bits_per_word);
int spi_set_extra_flags(cp_spi_t *spi, uint8_t extra_flags);

/* Miscellaneous */
int spi_fd(cp_spi_t *spi);
int spi_tostring(cp_spi_t *spi, char *str, size_t len);

/* Error Handling */
int spi_errno(cp_spi_t *spi);
const char *spi_errmsg(cp_spi_t *spi);
```
...
``` c
int spi_open_advanced(cp_spi_t *spi, const char *path, unsigned int mode, uint32_t max_speed,
...
`spi` should be a valid pointer to a SPI handle opened with `spi_open()` or `cp_spi_open_advanced()`.

### RETURN VALUE

The periphery SPI functions return 0 on success or one of the negative error codes below on failure.

The libc errno of the failure in an underlying libc library call can be obtained with the `cp_spi_errno()` helper function. A human readable error message can be obtained with the `spi_errmsg()` helper function.
...
int main(void) {
    ...
    if (cp_spi_open(spi, "/dev/spidev1.0", 0, 1000000) < 0) {
